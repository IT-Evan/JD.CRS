$(function () {
    ...
    $('#RefreshButton').click(function () {
        refreshDepartmentCourseList();
    });
    //call delete function
    $('.delete-departmentCourse').click(function () {
        var departmentCourseId = $(this).attr("data-departmentCourse-id");
        var departmentCourseName = $(this).attr('data-departmentCourse-name');
        deleteDepartmentCourse(departmentCourseId, departmentCourseName);
    });
    //call edit function
    $('.edit-departmentCourse').click(function (e) {
        var departmentCourseId = $(this).attr("data-departmentCourse-id");
        e.preventDefault();
        $.ajax({
            url: abp.appPath + 'DepartmentCourse/Edit?departmentCourseId=' + departmentCourseId,
            type: 'POST',
    ...
    //    location.href = '/DepartmentCourse?status=' + _$status.val() + '&keyword=' + _$keyword.val();
    //});
    ...
    function save() {
        ...
        _departmentCourseService.create(departmentCourse).done(function () {
            _$modal.modal('hide');
    ...
    function refreshDepartmentCourseList() {
        location.reload(true); //reload page to see new user!
    }
    //define delete function
    function deleteDepartmentCourse(departmentCourseId, departmentCourseName) {
        abp.message.confirm(
            abp.utils.formatString(abp.localization.localize('AreYouSureWantToDelete', 'CRS'), departmentCourseName),
            function (isConfirmed) {
                if (isConfirmed) {
                    _departmentCourseService.delete({
                        id: departmentCourseId
                    }).done(function () {
                        refreshDepartmentCourseList();
                    });
                }
